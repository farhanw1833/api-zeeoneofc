...
var creatorList = ['@farhanxz','@farhanxz'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'Need APIKEY?? Hubungi WhatsApp wa.me/628988554724 , Apikeynya gratis kok soalnya masih perkembangan'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'FarhanGanz') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'FarhanGanz') return res.json(loghandler.invalidKey)
